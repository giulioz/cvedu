...
const BlockTemplateRender = React.memo(function BlockTemplateRender<
  TBlockInfo,
  TPortInfo
>({
  type,
...
function searchTrueIOPort<TBlockInfo, TPortInfo>(
  port: IOPortInst<{}>,
  ...
  if (block) {
    ...
    const output =
      port.type === "output" && block.outputs.find(i => i.label === port.label);
    return input || output;
  } else {
    return null;
  }
